...
test('Incorrect artist name/no artist name in SongQuery', () => {
  expect.assertions(1);
  return expect(todo.querySong('Lift Yourself', '')).rejects.toEqual('Incorrect artist name');
});
...
test('tests searchForSong without an artist, should not work (pass)', () => {
    expect.assertions(1);
    return expect(todo.searchForSong('Lift Yourself', '')).rejects.toEqual('Could not find lyrics');
})